            ...
            if self._transitions[state]:
                for event in _orderedkeys(self._transitions[state],
                    ...
                    if target.on_enter is not None:
                        row.append(reflection.get_callback_name(target.on_enter))
                    else:
                        row.append(empty)
                    if target.on_exit is not None:
                        row.append(reflection.get_callback_name(target.on_exit))
                    else:
                        row.append(empty)
                    tbl.add_row(row)
            else:
                ...
                if on_enter is not None:
                    on_enter = reflection.get_callback_name(on_enter)
                else:
                    on_enter = empty
                on_exit = self._states[state]['on_exit']
                if on_exit is not None:
                    on_exit = reflection.get_callback_name(on_exit)
                else:
                    on_exit = empty
